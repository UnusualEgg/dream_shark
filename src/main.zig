const w4 = @import("wasm4.zig");
const std = @import("std");

const Shark = struct {
    x: i32 = 160,
    speed: i32 = @as(i32, @intFromFloat(surfer_max_speed)) / 2,
    alive: bool = true,

    pub const width = 49;
    pub const height = 36;
    pub const sprite_flags = w4.BLIT_2BPP;
    pub const sprite = [441]u8{  };

    fn update(self: *Shark, camera_offset: i32) void {
        self.x += self.speed;
        self.x += camera_offset;

        //stop the shark at 4 screens ahead
        self.x = std.math.clamp(self.x, -2 * 160, 4 * 160);
    }
    fn draw(self: *const Shark) void {
        w4.DRAW_COLORS.* = 0x4020;
        w4.blit(&sprite, self.x, 92, width, height, sprite_flags);
    }
};
const Surfer = struct {
    speed: f32 = 0,

    const x: i32 = 160 / 2;
    const y: i32 = 51;
    const speed_change: f32 = 0.225;

    const width = 26;
    const height = 26;
    const sprite_flags = w4.BLIT_1BPP;
    //3 angles sprites
    const sprite: [3][85]u8 = [3][85]u8{
        [85]u8{ 0x00, 0x3c, 0x00, 0x00, 0x0f, 0x07, 0x80, 0x03, 0xcf, 0xe0, 0x00, 0xf7, 0xe0, 0x00, 0x3f, 0xf0, 0x00, 0x1f, 0xf0, 0x00, 0x3f, 0xf0, 0x00, 0x0f, 0xfc, 0x00, 0x07, 0xff, 0x00, 0x03, 0xdf, 0xc0, 0x01, 0xf7, 0xf8, 0x00, 0x79, 0xfe, 0x00, 0x1e, 0x7f, 0xc0, 0x06, 0x0f, 0xf8, 0x01, 0x83, 0xff, 0x84, 0x00, 0xf3, 0xf7, 0x00, 0x3c, 0x7f, 0xc0, 0x0e, 0x0f, 0xf0, 0x07, 0x8f, 0xfc, 0x01, 0xff, 0xfe, 0x00, 0xff, 0xfc, 0x01, 0xff, 0xfe, 0x00, 0xff, 0xf8, 0x03, 0xff, 0xf0, 0x00, 0xff, 0xc0, 0x00, 0x3f, 0x80, 0x00, 0x00 },
        [85]u8{ 0x00, 0x00, 0x00, 0x00, 0x00, 0x78, 0x00, 0x00, 0x3e, 0x00, 0x00, 0x0f, 0x80, 0x00, 0x07, 0xc0, 0x00, 0x3f, 0xf6, 0x00, 0x3f, 0xff, 0xe0, 0x1f, 0x7f, 0xfe, 0x0e, 0x1f, 0xc3, 0xc3, 0x0f, 0xf0, 0x30, 0x03, 0xf8, 0x00, 0x00, 0xfe, 0x00, 0x00, 0x3f, 0x80, 0x00, 0x1f, 0xe0, 0x00, 0x07, 0xfc, 0x00, 0x03, 0xcf, 0x00, 0x33, 0xe1, 0xe0, 0x3f, 0xf8, 0x78, 0x0f, 0xff, 0xff, 0x03, 0xff, 0xff, 0xfc, 0x7f, 0xff, 0xff, 0xc0, 0x7f, 0xff, 0xf0, 0x00, 0x3f, 0xfc, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },
        [85]u8{ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0f, 0x00, 0x00, 0x07, 0xc0, 0x00, 0x01, 0xf0, 0x00, 0x7f, 0xf8, 0x00, 0x7f, 0xfe, 0x00, 0x1c, 0x3f, 0xf8, 0x00, 0x1f, 0xff, 0x80, 0x0f, 0xf0, 0xf0, 0x03, 0xfc, 0x0c, 0x00, 0xfe, 0x01, 0x00, 0x7f, 0x80, 0x00, 0x1f, 0xe0, 0x0f, 0x3f, 0x78, 0x03, 0xff, 0x8f, 0x00, 0xff, 0xe3, 0xc0, 0x3f, 0xff, 0xf0, 0x03, 0xff, 0xfe, 0x00, 0x0f, 0xff, 0xe0, 0x00, 0x7f, 0xfe, 0x00, 0x01, 0xff, 0xc0, 0x00, 0x0f, 0xf0, 0x00, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },
    };

    fn update(self: *Surfer, gamepad: u8) void {
        if (gamepad & w4.BUTTON_RIGHT != 0) {
            state.surfer.speed += speed_change;
        }
        if (gamepad & w4.BUTTON_LEFT != 0) {
            self.speed -= speed_change;
        }
        self.speed = std.math.clamp(self.speed, 0, surfer_max_speed);
    }
    fn draw(self: *const Surfer) void {
        w4.DRAW_COLORS.* = 0x0040;
        //3 equally spaced states of rotation
        //f(x) = (2x)/max
        const surfer_index: usize = @intFromFloat((self.speed * 2.0) / surfer_max_speed);
        //blit uses top left corner as origin
        //so move surfer half over
        w4.blit(&sprite[surfer_index], x - (width / 2), y, width, height, sprite_flags);
    }
};
const surfer_max_speed: f32 = 5;

const Water = struct {
    x: i32 = 0,

    const y: i32 = 60;
    const width = 160;
    const height = 12;
    const sprite_flags = w4.BLIT_1BPP;
    const sprite = [240]u8{  };

    fn update(self: *Water, camera_offset: i32) void {
        self.x +|= camera_offset;
        //move them forward to keep looping so we never run out of water
        while (self.x <= -width) {
            self.x += width;
        }
    }
    fn draw(self: *const Water) void {

        //only the top of the water uses a sprite because it's detailed
        w4.DRAW_COLORS.* = 0x0030;
        //we use two to make it look endless
        w4.blit(&sprite, self.x, y, width, height, sprite_flags);
        w4.blit(&sprite, self.x + width, y, width, height, sprite_flags);

        w4.DRAW_COLORS.* = 0x0003;
        w4.rect(0, y + height, width, 160 - (y + height));
    }
};

const Beam = struct {
    size: i32 = 0,

    const width: i32 = 50;

    fn update(self: *Beam, gamepad: u8, shark_x: i32, shark_alive: *bool) void {
        //shrink over time
        self.size -= 2;
        if (self.size < 0) {
            self.size = 0;
        }
        if (gamepad & w4.BUTTON_1 != 0 and self.size == 0 and shark_alive.*) {
            self.size = width;
            const hitbox_inset = 10;
            const shark_right = (shark_x + Shark.width) - hitbox_inset;
            const shark_left = shark_x + hitbox_inset;
            const screen_middle = 160 / 2;
            const beam_half = (width / 2);
            const beam_left = screen_middle - beam_half;
            const beam_right = screen_middle + beam_half;
            if (shark_right >= beam_left and shark_left <= beam_right) {
                shark_alive.* = false;
            }
        }
    }
    fn draw(self: *const Beam) void {
        if (self.size > 0) {
            w4.DRAW_COLORS.* = 0x0002;
            w4.rect(Surfer.x - @divFloor(self.size, 2), 0, @intCast(self.size), 160);
        }
    }
};
const State = struct {
    surfer: Surfer = .{},
    beam: Beam = .{},
    shark: Shark = .{},
    water: Water = .{},
    prev_button2_state: bool = false,
    moved: bool = false,

    fn update(self: *State) void {
        const gamepad = w4.GAMEPAD1.*;

        //reset button
        const curr_button2_state = gamepad & w4.BUTTON_2 != 0;
        if (curr_button2_state and !self.prev_button2_state and self.beam.size == 0) {
            self.shark.alive = true;
            self.surfer.speed = 0;
            self.shark.x = 160;
        }
        self.prev_button2_state = gamepad & w4.BUTTON_2 != 0;

        //update
        const camera_offset: i32 = if (self.shark.alive) @intFromFloat(-self.surfer.speed) else 0;
        self.beam.update(gamepad, self.shark.x, &self.shark.alive);
        if (self.shark.alive) {
            self.surfer.update(gamepad);
            self.shark.update(camera_offset);
            self.water.update(camera_offset);
        }

        if (gamepad & (w4.BUTTON_LEFT | w4.BUTTON_RIGHT | w4.BUTTON_1) != 0) self.moved = true;
    }
    fn draw(self: *const State) void {
        if (!self.moved) {
            w4.DRAW_COLORS.* = 2;
            const title: []const u8 = "My dream game :3";
            w4.text(title, (160 / 2) - ((title.len / 2) * w4.FONT_SIZE), 10);
        }

        self.water.draw();
        self.surfer.draw();
        self.shark.draw();
        self.beam.draw();
    }
};
var state: State = .{};

export fn start() void {
    w4.PALETTE.* = .{
        0x5995d1, //sky blue (light)
        0xffffff, //white
        0x1d456d, //water blue (dark)
        0x58dd58, //green
    };
}

//everything is moved by surfer_speed

export fn update() void {
    state.update();
    state.draw();
}
